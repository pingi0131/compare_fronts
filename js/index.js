...
async function loadFilesFromDataDir(year, month) {
    ...
    if (dataSource === 'EPM_H-MoQTS') {
        csvFiles = [
            `H-MoQTS/H-MoQTS/SMC_M2M/UA_${year}_${paddedMonth}%28${year}%20Q1%29_60%23_front.csv`,
    ...
    } else if (dataSource === 'Hybrid_ELS_BA') {
        csvFiles = [
            `Hybrid_BA_ELS/with ELS/train_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
            `Hybrid_BA_ELS/w.o. ELS/before_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
        ];
    } else if (dataSource === 'WPM_Hybird') {
        csvFiles = [
            `WPM-Hybrid/Hybrid/M2M/train_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
            `WPM-Hybrid/SMS-EMOA/SMS-EMOA_M2M_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
            `WPM-Hybrid/NSGA-II/${year}${paddedMonth}_exp1_pareto.csv`,
            `WPM-Hybrid/MOEAD/MOEAD_fund_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
        ];
    } else if (dataSource === 'WPM_MoQTS') {
        csvFiles = [
            `MoQTS/MoQTS/M2M/train_${year}_${paddedMonth}%28${year}%20Q1%29_50_front.csv`,
            `MoQTS/SMS-EMOA/SMS-EMOA_M2M_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
            `MoQTS/NSGA-II/${year}${paddedMonth}_exp1_pareto.csv`,
            `MoQTS/MOEAD/MOEAD_fund_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
        ];
    } else if (dataSource === 'WPM_MoQA') {
        csvFiles = [
            `WPM-MoQA/MoQA/M2M/before_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
            `WPM-MoQA/SMS-EMOA/SMS-EMOA_M2M_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
            `WPM-MoQA/NSGA-II/${year}${paddedMonth}_exp1_pareto.csv`,
            `WPM-MoQA/MOEAD/MOEAD_fund_${year}_${paddedMonth}%28${year}%20Q1%29.csv`,
        ];
    }
        ...
        for (const filePath of csvFiles) {
            ...
            try {
                ...
                if (parsed.length > 0) {
                    ...
                    //const color = (folder.includes('MoQTS') || folder === 'MoQA' || folder === 'with ELS' || folder === 'Our method' || folder === 'MoQIO' || folder === 'Hybrid') ? '#FF0000' : defaultColors[datasetMap.size - 1 % defaultColors.length] || '#000000'; // Adjust index
                    ...
                    datasetMap.set(fileName, {
                        ...
                        pointRadius: 2 // Add default point radius
                    });
...
function resetColors() {
    ...
    entries.forEach(([fileName, data], index) => {
        const newColor = priorityFolders.includes(data.name) ? '#FF0000' : defaultColors[index - 1 % defaultColors.length] || '#000000';
        //const newColor = (data.name.include('MoQTS') || data.name === 'MoQA' || data.name === 'with ELS' || data.name === 'Our method' || data.name === 'MoQIO' || data.name === 'Hybrid') ? '#FF0000' : defaultColors[index - 1 % defaultColors.length] || '#000000';
        data.color = newColor;
...
function generateMetricsTable() {
    ...
    // Sort metricsData to place MoQTS, MoQA, with ELS, Our method, MoQIO at the bottom
    //const priorityFolders = ['MoQTS', 'MoQA', 'with ELS', 'Our method', 'MoQIO', 'Hybrid'];
    metricsData.sort((a, b) => {
        const aIsPriority = priorityFolders.includes(a.customName);
        const bIsPriority = priorityFolders.includes(b.customName);
        if (aIsPriority && !bIsPriority) return 1; // a is priority, move to bottom
        if (!aIsPriority && bIsPriority) return -1; // b is priority, move to bottom
        return 0; // Maintain relative order for non-priority or both priority
    });

    const maxNDS = metricsData.length > 0 ? 0 : 0;
